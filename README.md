...
4. 按按钮提示完成页面上的滑动验证，再点击同一个按钮继续查询；后续每次提示时重复此操作。

活动累计查询会按最长 7 天分段，以尽量少的请求获取明细；每个分段和分页都需要单独完成一次人工滑动验证。没有记录的区间会跳过，全部查询完成后再按记录日期汇总。结果中的折线图展示从首个有记录日到今天、截至每日的累计盈亏，天数较多时可以横向滚动。回报率按 `收入 ÷ 消费 × 100%` 计算。
